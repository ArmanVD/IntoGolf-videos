...
| scorekaart buitenland | 19-scorekaart-buitenland.spec.js |
...
### tests/mobile-videos/19-scorekaart-buitenland.spec.js (6 clips, ~6.5 MB)
- Login silently, open menu, navigate to Handicap, click Buitenland tab twice (first to switch view, second to open form), fill date/time/country (België), click Volgende, fill club (Royal Antwerp Golf Club)/holes (18)/lus (Tom Simpson Course)/tee (Heren blauw), click Volgende, fill courserating (69.0)/sloperating (120)/par (73), click Volgende, fill stableford (36), click Opslaan, wait for success, click second Opslaan on summary
- Instructions: "Wanneer u ingelogd bent, start u op het dashboard. Klik linksboven op het menu-icoontje om het menu te openen.", "Klik vervolgens op 'Handicap' en dan op 'Buitenland'.", "Vul de datum, tijd en het land in waar gespeeld is. Klik vervolgens op 'Volgende'.", "Voer nu de naam van de baan in, het aantal holes, de naam van de lus en de kleur van de tee. Klik vervolgens op 'Volgende'.", "Vul nu de courserating, sloperating en de par in van de gespeelde baan. Klik vervolgens op 'Volgende'.", "Vul ten slotte het aantal Stableford-punten in, het GSN-nummer van de marker indien aanwezig, en eventuele opmerkingen. Klik vervolgens op 'Opslaan' om de scorekaart op te slaan."
- **Note:** Buitenland tab must be clicked TWICE — first click switches to Buitenland view (list), second opens new scorecard form; stepper form keeps ALL step inputs in the DOM simultaneously — page1=idx0-2 (date/time/country), page2=idx3-6 (club/holes-display/lus/tee-display), page3=idx7-9 (ratings), page4=idx10 (stableford); holes q-select=.nth(1), tee q-select=.nth(2); use { force: true } on q-select taps; two Opslaan clicks required (form submit → summary confirmation)

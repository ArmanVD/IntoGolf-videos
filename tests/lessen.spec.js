...
/**
 * Show a Dutch instruction overlay on the page.
 * Uses aria-hidden so Playwright selectors don't match the overlay text.
 */
...
test.describe("Lessen", () => {
  test("should book and cancel a privé les", async ({ page }) => {
    // Log in
    ...
    await page.waitForTimeout(3500); // TTS
    ...
    await page.waitForTimeout(3000); // TTS + wait for time slots to load
    ...
    await page.waitForTimeout(4000); // TTS + wait for toast to disappear
    ...
    await page.waitForTimeout(4000); // TTS
